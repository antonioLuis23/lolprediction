...
def getIndexTest(array):
    ...
    for i in range(array.shape[0]):
        #if array[i, 4, 50]>=0 and array[i,4,50]<=0.0532: #0 a 5 min
        #if array[i, 4, 50]>=0.05318 and array[i,4,50]<=0.1064: #5 a 10 min
        #if array[i, 4, 50]>=0.1062 and array[i,4,50]<=0.16: #10 a 15 min
        #if array[i, 4, 50]>=0.211 and array[i,4,50]<=0.266: #20 a 25 min
        if array[i, 4, 50]>=0 and array[i,4,50]<=0.1064: #0 a 10 min
            ind.append(i)
        #print(array[i, 4,:])
    return ind
...
def getIndexTrain(array):
    ...
    for i in range(array.shape[0]):
        #if array[i, 4, 50]>=0 and array[i,4,50]<=0.0532: #0 a 5 min
        #if array[i, 4, 50]>=0.05318 and array[i,4,50]<=0.1064: #5 a 10 min
        #if array[i, 4, 50]>=0.05318 and array[i,4,50]<=0.16: #10 a 15 min
        #if array[i, 4, 50]>=0.158 and array[i,4,50]<=0.213: #15 a 20 min
        #if array[i, 4, 50]>=0.211 and array[i,4,50]<=0.266: #20 a 25 min
        if array[i, 4, 50]>=0 and array[i,4,50]<=0.1064: #0 a 10 min
            ind.append(i)
    return ind
...
for train, test in kfold.split(min_inicios_tupla):
    ...
    filepath="10-15-weights-improvement-{epoch:02d}-{loss:.4f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1, save_best_only=True, mode='min')
    callbacks_list = [checkpoint]
    # fit the model
    model.fit(X_train, Y_train, epochs=50, batch_size=64, callbacks=callbacks_list, verbose = 1)
